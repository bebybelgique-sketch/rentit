# Sprint 1 — Definition of Done (merge point: 768d521)

**Точка развёртывания Sprint 1 — коммит `6122196`.** Разворачивать и
проверять надо именно его (`git checkout 6122196`, либо ветка
`sprint-1-deploy` от него): дальше в ветке идёт Sprint 2, и `npm run
test:edge` из более позднего кода проверял бы не то, что развёрнуто.
