 ...
 *
 * Раньше отсюда уходила смесь языков: 'Forbidden' рядом с
 * 'La réservation a changé entre-temps'. Клиент показывал это как есть —
...
serve(async (req) => {
  ...
  try {
    ...
    // занятости) и владельца (для проверки прав). price_per_day и deposit
    // отсюда убраны намеренно — не потому, что лишний столбец дорог, а
