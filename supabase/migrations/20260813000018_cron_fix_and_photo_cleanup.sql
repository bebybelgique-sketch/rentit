...
--    статусом `failed`, ноль успешных. Причин оказалось ТРИ, и главная —
--    последняя; первые две я нашёл раньше и чуть не остановился на них:
...
--      — **расширения `pg_net` в базе не было вовсе**. Все 718 отказов —
--        дословно `ERROR: schema "net" does not exist`. То есть до адреса и
--        до ключа дело не доходило ни разу: команда падала на разборе
--        запроса. `pg_cron` стоял, `pg_net` числился доступным, но не
--        установленным. Ставится ниже, первой же строкой.
...
--
--    УРОК: сообщение об ошибке лежало в `cron.job_run_details` 718 раз и
--    называло причину дословно. Я же вычитал две ошибки ГЛАЗАМИ из текста
--    команды и принял их за полный ответ. Читать надо было отказ, а не код.
--
...
-- --------------------------------------------------------------------------
-- 0. Расширение, без которого всё остальное бессмысленно
-- --------------------------------------------------------------------------
--
-- `net.http_post` живёт в pg_net. Без него любое задание планировщика падает
-- на разборе запроса, не дойдя ни до адреса, ни до заголовков — именно это и
-- происходило 718 раз подряд с апреля.
