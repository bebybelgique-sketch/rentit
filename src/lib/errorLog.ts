...
// ── ЧТО ЭТО РЕШАЕТ ───────────────────────────────────────────────────
//
// С 23.09 каждая запись уходит и на сервер (errorReport.ts → функция
// report-error → таблица client_errors, миграция 41): владелец продукта
// узнаёт о поломке у чужого человека, даже если тот ничего не прислал.
// До этого здесь было честно написано, что не узнаёт.
//
// И на устройстве: когда у человека что-то ломается, он видит не пустой экран, а
...
export function logError(source: LoggedError['source'], error: unknown): void {
  ...
  // И на сервер — сама, без действия человека (см. errorReport.ts).
  reportError(entry)
