...
interface Props {
  ...
  status: string;
  /**
   * Статус, который вернул СЕРВЕР (не тот, что компонент ожидал по своей таблице
   * переходов). Согласованность кэша от этого колбэка больше НЕ зависит:
   * хуки инвалидируют ['bookings'] и ['items'] сами (src/lib/queryKeys.ts),
   * поэтому /my-items с 06.09 его не передаёт и не правит строки руками —
   * раньше страница подменяла статус в своём кэше и устаревала при любом
   * действии с другой страницы.
   */
