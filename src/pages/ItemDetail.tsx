...
export default function ItemDetail() {
  ...
  // Ссылкой делятся С КОДОМ приглашения поделившегося: сосед, который
  // зарегистрируется по ней, засчитается ему (src/lib/referral.ts). Без
  // входа кода нет — ссылка уходит как есть.
  const invite = useMyInvite(user?.id)
  const shareUrl = withReferral(window.location.href, invite.data?.code)
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <WhatsAppLink
              ...
              text={`${t('share.whatsappText', { title: item.title, price: item.price_per_day })}\n${shareUrl}`}
