...
test.describe('поиск по близости', () => {
  ...
  test('вещь вне радиуса отсекает база — до браузера она не доезжает', async ({ page, context }) => {
    ...
    // И ЗДЕСЬ ЗОНУ ВИНЯТ ПО ДЕЛУ. Это единственное место набора, где она
    // применена по-настоящему: близость включена, координаты известны,
    // `p_radius_km` ушёл в базу. Витрина с 20.09 винит зону ТОЛЬКО в этом
    // случае — в остальных запрос глобальный, и причина другая. Проверка
    // стережёт сохранённую половину развилки: потерять её было бы так же
    // неверно, как оставить прежнее поведение целиком.
