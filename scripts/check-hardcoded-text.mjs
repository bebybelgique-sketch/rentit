...
// Текст между тегами.
//
// `(?<!=)` — НЕ КОСМЕТИКА. Стрелка функции тоже кончается на `>`, и без
// этого исключения обычная строка TypeScript
//
//     const focusable = () => [...node.querySelectorAll<HTMLElement>(SEL)]
//
// читалась как «текст `[...node.querySelectorAll` между тегом `=>` и
// тегом `<HTMLElement>`». То есть гейт объявлял подписью интерфейса
// параметр обобщённого типа.
//
// Исключение для стрелки в этом файле уже было — `endsWithTag` ниже, —
// но применялось только к зонам 1 и 1б, где тег и текст стоят на РАЗНЫХ
// строках. В зоне, где `=>` встречается чаще всего — на одной строке с
// текстом, — его не было.
//
...
// Второй случай той же породы: `>=`. Строка
//
//     const past = ds >= startDate && ds <= end
//
// читалась как «текст `= startDate && ds` между тегами», и этот мусор
// лежал в замороженном списке как настоящее исключение. `(?!=)` после
// `>` его убирает: подпись интерфейса не начинается со знака равенства
// ни в одном из трёх языков продукта.
