...
// Себя гейт не просматривает: в карте RETIRED снятые цвета НАЗВАНЫ, это
// и есть определение запрета. Найдя их там, он сообщал бы о нарушении в
// собственном тексте — и каждый, кто открыл бы отчёт, учился бы не
// верить отчёту.
const SKIP = ['/__tests__/', '/test/', 'scripts/check-retired-palette.mjs']

/**
 * Снять комментарии: они хранят причину снятия и запретом не считаются.
 *
 * HTML-комментарии сюда добавлены 21.09 вместе с просмотром index.html.
 * Первый же прогон дал там «нарушение» — им оказалось пояснение, ПОЧЕМУ
 * цвет системной полосы больше не #080808. Запрещать такие записи значит
 * стирать причину, по которой правку не повторят обратно.
 */
 ...
 * Где ищем.
 *
 * ДО 21.09 ЗДЕСЬ БЫЛ ТОЛЬКО src/ — и шапка этого файла на том основании
 * утверждала: «#ADFF2F после 19.09 не осталось нигде». Утверждение было
 * ложным, и опровергалось оно в два шага: иконка приложения
 * (public/icons/) и картинка превью носили ровно тот лайм на ровно том
 * чёрном. Полтора месяца их видел каждый, кто ставил сайт на домашний
 * экран, — и не видел ни один сторож.
 *
 * Сторож со слепой зоной опаснее отсутствующего: он выдаёт разрешение,
 * которого не проверял.
 *
 * ЧЕГО НЕ ВИДНО И СЕЙЧАС. Растр. В .png цвет лежит пикселями, и поиском
 * по тексту его не найти. Закрыто это не проверкой файла, а тем, что
 * растры БОЛЬШЕ НЕ РИСУЮТ РУКАМИ: их собирает scripts/build-brand-assets.mjs,
 * а он обычный текст и просматривается наравне с остальным. Цвета,
 * которого нет в источнике, не может оказаться в результате.
 */
const SCAN = [
  { dir: 'src', ext: /\.(ts|tsx|css)$/ },
  { dir: 'scripts', ext: /\.mjs$/ },
  { dir: 'public', ext: /\.(svg|json|webmanifest)$/ },
]

const scannedFiles = () => [
  ...SCAN.flatMap(({ dir, ext }) => walk(join(root, dir)).filter((f) => ext.test(f)).map(rel)),
  'index.html',
